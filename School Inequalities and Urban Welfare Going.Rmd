...
clustering <- function(datafile) {
  
  data <- 
    data.frame(CodMunicipio =
                 datafile$CodMunicipio,
               r = datafile$LPCC$LPCC,
               p = datafile$LPCC$LPCC_sig_BF) 
  ...
  thisClustering <-
    data.frame(
        CodMunicipio = data$CodMunicipio,
        r = data$r,
        Cluster = thisMoran$Cluster,
        p.rand = thisMoran$p.value,
        stringsAsFactors = FALSE
      )

  RPushbullet::pbPost(
    type = "note",
    title = paste(deparse(substitute(datafile)),
                  "clustering", 
                  "finished!"),
    apikey = "o.SYjKv7goGyRzOWif4k8hwA6wPw1Hg00q",
    devices = "Galaxy A8"
...
D1IDEBCluster <- clustering(D1IDEBLPCC)
D5IDEBCluster <- clustering(D5IDEBLPCC)
